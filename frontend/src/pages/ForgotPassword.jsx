...
function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrors({ email: data.message || 'Unable to process request' });
        return;
      }
      ...
      setErrors({ email: 'Unable to connect. Please try again.' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ width: '100%', maxWidth: '390px', background: '#ffffff', borderRadius: '18px', padding: '28px 24px', boxShadow: '0 8px 28px rgba(15, 23, 42, 0.08)', border: '1px solid #e2e8f0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '24px' }}>
          ...
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#0f172a', marginBottom: '6px' }}>Reset Password</h1>
          <p style={{ fontSize: '13px', color: '#64748b', textAlign: 'center' }}>Enter your email to receive password reset instructions</p>
        </div>

        {/* SUCCESS MESSAGE */}
        {submitted && (
          <div style={{ background: '#ECFDF5', color: '#059669', border: '1px solid #86EFAC', padding: '12px 14px', borderRadius: '10px', fontSize: '12px', marginBottom: '16px', textAlign: 'center' }}>
            ✓ {successMessage}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <button
            ...
            style={{
              width: '100%',
              padding: '11px',
              background: submitted ? '#10b981' : '#0EA5E9',
              color: 'white',
              border: 'none',
              borderRadius: '10px',
              fontSize: '14px',
              ...
              transition: 'all 0.2s',
              boxShadow: submitted ? '0 4px 12px rgba(16,185,129,0.3)' : '0 4px 12px rgba(14,165,233,0.3)',
            }}
          >
